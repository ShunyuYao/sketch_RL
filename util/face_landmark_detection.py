...
def face_landmark_detection(dataset_path, predictor_path, save_path, phase='train'):
    ...
    for i in range(len(img_paths)):
        ...
        save_path = os.path.join(save_path, phase + '_keypoints', os.path.basename(f))
        if not os.path.isdir(save_path):
            os.makedirs(save_path)

        for img_name in sorted(glob.glob(os.path.join(f, '*.bmp'))):
